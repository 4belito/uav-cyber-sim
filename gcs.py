...
class GCS(Enviroment):
    ...
    def get_nearest_neighbor(self, uav):
        target_position = local2global(
            uav.curr_pos, uav.home, pairwise=True
        )  # Get the position of the target ID
    ...
    def send_neighborg_position(self, uav: VehicleLogic, neighborg_id: int):
        ...
        loacl_obj_pos = global2local(
            self.poss[neighborg_id - 1], uav.home, pairwise=True
        )
        next_wp = GCS.get_avoidance_wp(
            pos=uav.curr_pos, obj_pos=loacl_obj_pos, goal_pos=wp
        )
