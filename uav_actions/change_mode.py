...
def set_guided_mode(conn: mavutil.mavlink_connection, blocking: bool = False) -> None:
    """
...
def make_set_guided_mode():
    mode_guided_step = Step("Change to Guided Mode",check_fn=check_guided_mode,exec_fn=set_guided_mode)

    mode_guided = Action("Guided Mode")
    mode_guided.add_step(mode_guided_step)
    return mode_guided